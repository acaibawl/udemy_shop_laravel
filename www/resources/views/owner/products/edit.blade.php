<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semib$product->extl text-gray-800 leading-tight">
            {{ __('Dashboard') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="post" action="{{ route('owner.products.destroy', ['product' => $product->id]) }}" id="delete_{{ $product->id }}">
                        ...
                        <div class="p-2 w-full flex justify-around mt-32">
                            <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" type="button" class="text-white bg-red-400 border-0 py-2 md:px-4 focus:outline-none hover:bg-red-500 rounded">削除する</a>
                        </div>
                    </form>
                </div>
            </div>
