            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="md:p-6 text-gray-900">
                            ...
                            <div class="lg:w-2/3 w-full mx-auto overflow-auto">
                                ...
                                <table class="table-auto w-full text-left whitespace-no-wrap">
                                    ...
                                    <tr>
                                        ...
                                        <th class="md:px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tr rounded-br">編集</th>
                                        <th class="md:px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tr rounded-br">削除</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($owners as $owner)
                                        <tr>
                                            ...
                                            <td class="md:px-4 py-3">
                                                <button onclick="location.href='{{ route('admin.owners.edit', ['owner' => $owner->id]) }}'" type="button" class="text-white bg-indigo-400 border-0 py-2 md:px-4 focus:outline-none hover:bg-indigo-500 rounded">編集</button>
                                            </td>
                                            <td class="md:px-4 py-3">
                                                <form method="post" action="{{ route('admin.owners.destroy', ['owner' => $owner->id]) }}" id="delete_{{ $owner->id }}">
                                                    @method('DELETE')
                                                    @csrf
                                                    <a href="#" data-id="{{ $owner->id }}" onclick="deletePost(this)" type="button" class="text-white bg-red-400 border-0 py-2 md:px-4 focus:outline-none hover:bg-red-500 rounded">削除</a>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                    ...
                    @foreach($q_get as $q_owner)
                        {{ $q_owner->name }}
                        {{ Carbon::parse($q_owner->created_at)->diffForHumans() }}
                    @endforeach --}}
                </div>
            </div>
        </div>
